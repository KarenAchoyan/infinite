...
function* fetchSingleStaffSaga(action) {
  try {
    ...
    const staff = yield call(() => axiosInstance.get(`/staff/show/${id}`, action.payload));
...
function* addStaffSaga(action) {
  try {
    const newStaff = yield call(() => axiosInstance.post('/staff/store', action.payload));
...
function* updateStaffSaga(action) {
  try {
    const { id } = action.payload;
    const updatedStaff = yield call(() => axiosInstance.post(`/staff/update/${id}`, action.payload));
...
function* deleteStaffSaga(action) {
  try {
    ...
    yield call(() => axiosInstance.get(`/staff/delete/${staffId}`));
