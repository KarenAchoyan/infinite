...
function* handleGetBlogs({ payload = {} }) {
  try {
    const response = yield call(() => axiosInstance.get('/blog/all', payload));
    const blogs = response.data; // Assuming the categories data is in response.data
    yield put(getBlogs.success(blogs.blogs));
  } catch (error) {
...
function* handleAddBlog(action) {
  try {
    const newBlog = yield call(() => axiosInstance.post(`/blog/store`, action.payload));
...
function* handleDeleteBlog(action) {
  try {
    ...
    yield call(() => axiosInstance.get(`/blog/delete/${blogId}`));
